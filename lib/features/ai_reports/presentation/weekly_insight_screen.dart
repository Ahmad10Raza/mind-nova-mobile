...
class WeeklyInsightScreen extends StatelessWidget {
            ...
            onPressed: () async {
              try {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Re-generating report...')),
                );
                await ref.read(triggerWeeklyReportProvider(null).future);
                ref.invalidate(weeklyReportProvider);
                ref.invalidate(weeklyReportHistoryProvider);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Report updated! Return to history to view.')),
                );
              } catch (e) {
                ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildMetricsGrid() {
    return GridView.count(
      ...
      children: [
        ...
        _metricCard('Sleep', report.avgSleepHours != null ? report.avgSleepHours!.toStringAsFixed(1) : '—', 'hrs', Icons.bedtime_rounded, const Color(0xFF7C4DFF)),
        _metricCard('Volatility', report.emotionalVolatility != null ? report.emotionalVolatility!.toStringAsFixed(1) : '—', 'σ', Icons.show_chart_rounded, _volColor(report.emotionalVolatility)),
  ...
  Widget _winChip(String text, Color color) {
    return Container(
      ...
      child: Text(text, style: AppTypography.caption.copyWith(fontSize: 12, fontWeight: FontWeight.w600, color: color)),
    );
  }
